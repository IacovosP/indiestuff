...
import { AlbumOrSingle, AlbumPageInterface, TrackInterface } from '@src/app/music-types/lib';
...
export class ArtistCreationPageComponent implements OnInit {
    ...
    constructor(public dialog: MatDialog, private playerSharedService: SharedService, private router: Router) {
    ...
    drop(event: CdkDragDrop<string[]>) {
        const prevItem = event.currentIndex !== 0 ? this.tracks[event.currentIndex].positionInAlbum : '';
        let nextItem: string;
        if (event.currentIndex === 0) {
            nextItem = event.currentIndex !== this.tracks.length - 1 ? this.tracks[event.currentIndex].positionInAlbum : '';
        } else {
            nextItem = event.currentIndex !== this.tracks.length - 1 ? this.tracks[event.currentIndex + 1].positionInAlbum : '';
        }
        moveItemInArray(this.tracks, event.previousIndex, event.currentIndex);
        this.tracks[event.currentIndex].positionInAlbum = midString(prevItem, nextItem);
    }

    ngOnInit() {
        this.subscription = playerEventEmitter.getEmittedValue().subscribe((item: PlayerChangeEvent) => this.changeIndexOfSongPlaying(item));
    ...
    loadAlbumForEdit(albumId: string) {
        defaultHttpClient
            ...
            .then((response: AlbumPageInterface) => {
                ...
                this.albumForEdit.album_image_filename = 'https://indie-image-test.s3.eu-west-2.amazonaws.com/' + response.album_image_filename;
                this.albumForEdit.artist.artist_image_filename = 'https://indie-artist-image-test.s3.eu-west-2.amazonaws.com/' + response.artist.artist_image_filename;
    ...
    openTrackUploadForm(trackToEditIndex?: number) {
        ...
        this.dialogRefClassScope = dialogRef;
        dialogRef.afterClosed().subscribe(async (result) => {
            const pendingUploadsLength = this.pendingUploads ? this.pendingUploads.length : 0;
            ...
            if (track && track.title && track.filename) {
                ...
                if (!trackToEdit) {
                    track.positionInAlbum = this.tracks.length === 0 ? midString() : midString(this.tracks[this.tracks.length - 1].positionInAlbum);
                    ...
                    this.albumForEdit.durationInSec = this.albumForEdit.durationInSec - durationOfTrackBeingReplaced + track.durationInSec;
                }
            }
        });
    }
    ...
    onFormSubmit({ value, valid }: { value: any; valid: boolean }) {
        ...
        if (this.albumForEdit) {
            ...
            defaultHttpClient
                .fetch('album/edit', JSON.stringify({ editedAlbum: albumForEditForRequest }), 'POST')
                .then((response) => {
            ...
            defaultHttpClient
                .fetch('album/create', JSON.stringify({ newAlbum: this.newAlbum }), 'POST')
                .then((response) => {
                    this.router.navigate(['/album', response.albumId]);
                })
                .catch((err) => {
    ...
    private setTrackList(album: AlbumPageInterface) {
        const sortedTracks = album.tracks.sort((track1, track2) => track1.positionInAlbum.localeCompare(track2.positionInAlbum));
