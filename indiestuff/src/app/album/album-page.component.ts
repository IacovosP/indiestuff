...
export class AlbumPageComponent implements OnInit {
    ...
    loadPage(albumId: string) {
        defaultHttpClient
            ...
            .then((response: AlbumPageInterface) => {
                this.album = response;
                this.album.album_image_filename = response.album_image_filename && 'https://indie-image-test.s3.eu-west-2.amazonaws.com/' + response.album_image_filename;
                this.album.artist.artist_image_filename =
                    response.artist.artist_image_filename && 'https://indie-artist-image-test.s3.eu-west-2.amazonaws.com/' + response.artist.artist_image_filename;
    ...
    private setAlbumDescription(album: AlbumPageInterface) {
        ...
        this.albumDescription = {
            ...
            durationInSec,
            releaseDate: `${getMonthName(releaseDate.getMonth())} ${releaseDate.toISOString().substr(8, 2)} • ${releaseDate.toISOString().substr(0, 4)}`
        };
    }
}
