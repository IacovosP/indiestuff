...
export class HttpClient {
        ...
        if (retryOptions) {
            return pRetry(executeFetch, {
                onFailedAttempt: async (error) => {
                    console.log(`[HttpError][Url: ${url}][Payload: ${body}] - Attempt ${error.attemptNumber} failed. There are ${error.retriesLeft} retries left.`);
    ...
    public fromFetch(url: string, body?: string | FormData, method: 'GET' | 'POST' = 'GET', headers?: Headers) {
        const requestUrl = `${REST_URL_DEV}/${url}`;
        let requestInit: RequestInit = {
            body,
            headers,
