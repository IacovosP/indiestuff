...
import httpClient, { HttpClient as HttpClientInterface } from '@src/app/network-core/HttpClient';
...
class DefaultHttpClient {
    ...
    public async fetch(
        ...
        const match = POST_NO_AUTH_URL.filter((noAuthUrl) => url.includes(noAuthUrl));
        if ((method === 'POST' || method === 'DELETE') && !accessToken && !match) {
            Promise.reject('Authenticated POST request for unauthenticated user');
        }
        return this.defaultHttpClient.fetch(url, body, method, retryOptions, this.getHeaders(body instanceof FormData, accessToken));
    }
    public fromFetch(url: string, body?: string | FormData, method?: 'GET' | 'POST'): Observable<any> {
        return this.defaultHttpClient.fromFetch(url, body, method, this.getHeaders(body instanceof FormData));
    }
    ...
    public getAccessToken(): Promise<string | null> {
        ...
        if (tokenResponse) {
            if (tokenResponse.expiresAt - Date.now() < 30000) {
                console.log('refreshing because expiring in :' + (tokenResponse.expiresAt - Date.now()));
